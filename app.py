...
# Customize ticks
ax = plt.gca()
xticks = np.linspace(0, len(strike_prices)-1, 5)
yticks = np.linspace(0, len(volatilities)-1, 5)
ax.set_xticks(xticks)
ax.set_xticklabels(np.round(strike_prices[xticks].astype(float), 2))
ax.set_yticks(yticks)
ax.set_yticklabels(np.round(volatilities[yticks].astype(float), 2))
